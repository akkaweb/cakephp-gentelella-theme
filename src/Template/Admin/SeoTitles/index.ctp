...
<!-- Main content -->
  <div class="row">
    <div class="col-xs-12">
      <div class="x_panel">
          <div class="x_title">
              <h2>Seo Title <small><?= __d('gentelella','Index') ?></small></h2>
          ...
          <table class="table table-hover table-striped">
            ...
            <?php foreach ($seoTitles as $seoTitle): ?>
              <tr>
                ...
                <td><?= $this->Number->format($seoTitle->seo_uri_id) ?></td>
                <td><?= h($seoTitle->title) ?></td>
                <td><?= h($seoTitle->created) ?></td>
                <td><?= h($seoTitle->modified) ?></td>
                <td class="actions" style="white-space:nowrap">
                  <?= $this->Html->link(__d('gentelella','Edit'), ['action' => 'edit', $seoTitle->id], ['class'=>'btn btn-primary btn-xs']) ?>
                  <?= $this->Form->postLink(__d('gentelella','Delete'), ['action' => 'delete', $seoTitle->id], ['confirm' => __d('gentelella','Confirm to delete this entry?'), 'class'=>'btn btn-danger btn-xs']) ?>
                </td>
              </tr>
            <?php endforeach; ?>
          </table>
