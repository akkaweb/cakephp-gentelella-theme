...
<!-- Main content -->
  <div class="row">
    <div class="col-xs-12">
      <div class="x_panel">
          <div class="x_title">
              <h2>Testimonial <small><?= __d('gentelella','Index') ?></small></h2>
        ...
        <div class="x_content">
          ...
          <table class="table table-hover table-striped">
            <tr>
              ...
              <th><?= $this->Paginator->sort('img_dir') ?></th>
            ...
            <?php foreach ($testimonials as $testimonial): ?>
              <tr>
                ...
                <td><?= $this->Number->format($testimonial->weight) ?></td>
                <td><?= h($testimonial->active) ?></td>
                <td><?= h($testimonial->img) ?></td>
                <td><?= h($testimonial->img_dir) ?></td>
                <td><?= h($testimonial->created) ?></td>
                <td class="actions" style="white-space:nowrap">
                  <?= $this->Html->link(__d('gentelella','Edit'), ['action' => 'edit', $testimonial->id], ['class'=>'btn btn-primary btn-xs']) ?>
                  <?= $this->Form->postLink(__d('gentelella','Delete'), ['action' => 'delete', $testimonial->id], ['confirm' => __d('gentelella','Confirm to delete this entry?'), 'class'=>'btn btn-danger btn-xs']) ?>
                </td>
              </tr>
            <?php endforeach; ?>
          </table>
