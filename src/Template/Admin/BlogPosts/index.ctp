...
<!-- Main content -->
  <div class="row">
    <div class="col-xs-12">
      <div class="x_panel">
          <div class="x_title">
              <h2>Blog Post <small><?= __d('gentelella','Index') ?></small></h2>
        ...
        <div class="x_content">
          ...
          <table class="table table-hover table-striped">
            <tr>
              ...
              <th><?= $this->Paginator->sort('blog_category_id') ?></th>
            ...
            <?php foreach ($blogPosts as $blogPost): ?>
              <tr>
                ...
                <td><?= $this->Number->format($blogPost->blog_category_id) ?></td>
                <td><?= $this->Number->format($blogPost->tag_count) ?></td>
                <td><?= h($blogPost->title) ?></td>
                <td><?= h($blogPost->slug) ?></td>
                <td><?= h($blogPost->photo) ?></td>
                <td><?= h($blogPost->photo_dir) ?></td>
                <td class="actions" style="white-space:nowrap">
                  <?= $this->Html->link(__d('gentelella','Edit'), ['action' => 'edit', $blogPost->id], ['class'=>'btn btn-primary btn-xs']) ?>
                  <?= $this->Form->postLink(__d('gentelella','Delete'), ['action' => 'delete', $blogPost->id], ['confirm' => __d('gentelella','Confirm to delete this entry?'), 'class'=>'btn btn-danger btn-xs']) ?>
                </td>
              </tr>
            <?php endforeach; ?>
          </table>
