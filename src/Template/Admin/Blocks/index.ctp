...
<!-- Main content -->
  <div class="row">
    <div class="col-xs-12">
      <div class="x_panel">
          <div class="x_title">
              <h2>Block <small><?= __d('gentelella','Index') ?></small></h2>
        ...
        <div class="x_content">
          ...
          <table class="table table-hover table-striped">
            <tr>
              ...
              <th><?= $this->Paginator->sort('show_title') ?></th>
            ...
            <?php foreach ($blocks as $block): ?>
              <tr>
                ...
                <td><?= $this->Number->format($block->region_id) ?></td>
                <td><?= $this->Number->format($block->show_title) ?></td>
                <td><?= h($block->slug) ?></td>
                <td><?= h($block->title) ?></td>
                <td><?= $this->Number->format($block->position) ?></td>
                <td><?= $this->Number->format($block->status) ?></td>
                <td class="actions" style="white-space:nowrap">
                  <?= $this->Html->link(__d('gentelella','Edit'), ['action' => 'edit', $block->id], ['class'=>'btn btn-primary btn-xs']) ?>
                  <?= $this->Form->postLink(__d('gentelella','Delete'), ['action' => 'delete', $block->id], ['confirm' => __d('gentelella','Confirm to delete this entry?'), 'class'=>'btn btn-danger btn-xs']) ?>
                </td>
              </tr>
            <?php endforeach; ?>
          </table>
