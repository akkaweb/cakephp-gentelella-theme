...
<!-- Main content -->
  <div class="row">
    <div class="col-xs-12">
      <div class="x_panel">
          <div class="x_title">
              <h2>Seo Canonical <small><?= __d('gentelella','Index') ?></small></h2>
          ...
          <table class="table table-hover table-striped">
            ...
            <?php foreach ($seoCanonicals as $seoCanonical): ?>
              <tr>
                ...
                <td><?= $this->Number->format($seoCanonical->seo_uri_id) ?></td>
                <td><?= h($seoCanonical->canonical) ?></td>
                <td><?= h($seoCanonical->active) ?></td>
                <td class="actions" style="white-space:nowrap">
                  <?= $this->Html->link(__d('gentelella','Edit'), ['action' => 'edit', $seoCanonical->id], ['class'=>'btn btn-primary btn-xs']) ?>
                  <?= $this->Form->postLink(__d('gentelella','Delete'), ['action' => 'delete', $seoCanonical->id], ['confirm' => __d('gentelella','Confirm to delete this entry?'), 'class'=>'btn btn-danger btn-xs']) ?>
                </td>
              </tr>
            <?php endforeach; ?>
          </table>
