...
<!-- Main content -->
  <div class="row">
    <div class="col-xs-12">
      <div class="x_panel">
          <div class="x_title">
              <h2>Album <small><?= __d('gentelella','Index') ?></small></h2>
        ...
        <div class="x_content">
          ...
          <table class="table table-hover table-striped">
            <tr>
              ...
              <th><?= $this->Paginator->sort('photos_count') ?></th>
            ...
            <?php foreach ($albums as $album): ?>
              <tr>
                ...
                <td><?= h($album->name) ?></td>
                <td><?= h($album->slug) ?></td>
                <td><?= $this->Number->format($album->photos_count) ?></td>
                <td><?= h($album->status) ?></td>
                <td><?= h($album->created) ?></td>
                <td><?= h($album->modified) ?></td>
                <td class="actions" style="white-space:nowrap">
                  <?= $this->Html->link(__d('gentelella','Edit'), ['action' => 'edit', $album->id], ['class'=>'btn btn-primary btn-xs']) ?>
                  <?= $this->Form->postLink(__d('gentelella','Delete'), ['action' => 'delete', $album->id], ['confirm' => __d('gentelella','Confirm to delete this entry?'), 'class'=>'btn btn-danger btn-xs']) ?>
                </td>
              </tr>
            <?php endforeach; ?>
          </table>
