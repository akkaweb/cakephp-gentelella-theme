...
<!-- Main content -->
  <div class="row">
    <div class="col-xs-12">
      <div class="x_panel">
          <div class="x_title">
              <h2>Business Category <small><?= __d('gentelella','Index') ?></small></h2>
        ...
        <div class="x_content">
          ...
          <table class="table table-hover table-striped">
            <tr>
              ...
              <th><?= $this->Paginator->sort('business_count') ?></th>
            ...
            <?php foreach ($businessCategories as $businessCategory): ?>
              <tr>
                ...
                <td><?= $this->Number->format($businessCategory->business_count) ?></td>
                <td><?= h($businessCategory->created) ?></td>
                <td><?= h($businessCategory->modified) ?></td>
                <td class="actions" style="white-space:nowrap">
                  <?= $this->Html->link(__d('gentelella','Edit'), ['action' => 'edit', $businessCategory->id], ['class'=>'btn btn-primary btn-xs']) ?>
                  <?= $this->Form->postLink(__d('gentelella','Delete'), ['action' => 'delete', $businessCategory->id], ['confirm' => __d('gentelella','Confirm to delete this entry?'), 'class'=>'btn btn-danger btn-xs']) ?>
                </td>
              </tr>
            <?php endforeach; ?>
          </table>
