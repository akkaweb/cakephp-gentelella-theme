...
<!-- Main content -->
  <div class="row">
    <div class="col-xs-12">
      <div class="x_panel">
          <div class="x_title">
              <h2>Slide <small><?= __d('gentelella','Index') ?></small></h2>
        ...
        <div class="x_content">
          ...
          <table class="table table-hover table-striped">
            <tr>
              ...
              <th><?= $this->Paginator->sort('img_src') ?></th>
            ...
            <?php foreach ($slides as $slide): ?>
              <tr>
                <td><?= h($slide->id) ?></td>
                <td><?= h($slide->slider_id) ?></td>
                <td><?= h($slide->title) ?></td>
                <td><?= h($slide->img_src) ?></td>
                <td><?= h($slide->link) ?></td>
                <td><?= h($slide->class) ?></td>
                <td><?= h($slide->identifier) ?></td>
                <td class="actions" style="white-space:nowrap">
                  <?= $this->Html->link(__d('gentelella','Edit'), ['action' => 'edit', $slide->id], ['class'=>'btn btn-primary btn-xs']) ?>
                  <?= $this->Form->postLink(__d('gentelella','Delete'), ['action' => 'delete', $slide->id], ['confirm' => __d('gentelella','Confirm to delete this entry?'), 'class'=>'btn btn-danger btn-xs']) ?>
                </td>
              </tr>
            <?php endforeach; ?>
          </table>
