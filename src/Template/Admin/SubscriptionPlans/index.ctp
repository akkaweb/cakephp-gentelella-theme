...
<!-- Main content -->
  <div class="row">
    <div class="col-xs-12">
      <div class="x_panel">
          <div class="x_title">
              <h2>Subscription Plan <small><?= __d('gentelella','Index') ?></small></h2>
        ...
        <div class="x_content">
          ...
          <table class="table table-hover table-striped">
            <tr>
              ...
              <th><?= $this->Paginator->sort('plan_idx') ?></th>
            ...
            <?php foreach ($subscriptionPlans as $subscriptionPlan): ?>
              <tr>
                ...
                <td><?= h($subscriptionPlan->plan_idx) ?></td>
                <td><?= h($subscriptionPlan->name) ?></td>
                <td><?= h($subscriptionPlan->amount) ?></td>
                <td><?= h($subscriptionPlan->currency) ?></td>
                <td><?= h($subscriptionPlan->interval) ?></td>
                <td><?= h($subscriptionPlan->created) ?></td>
                <td class="actions" style="white-space:nowrap">
                  <?= $this->Html->link(__d('gentelella','Edit'), ['action' => 'edit', $subscriptionPlan->id], ['class'=>'btn btn-primary btn-xs']) ?>
                  <?= $this->Form->postLink(__d('gentelella','Delete'), ['action' => 'delete', $subscriptionPlan->id], ['confirm' => __d('gentelella','Confirm to delete this entry?'), 'class'=>'btn btn-danger btn-xs']) ?>
                </td>
              </tr>
            <?php endforeach; ?>
          </table>
