...
<!-- Main content -->
  <div class="row">
    <div class="col-xs-12">
      <div class="x_panel">
          <div class="x_title">
              <h2>Region <small><?= __d('gentelella','Index') ?></small></h2>
          ...
          <table class="table table-hover table-striped">
            ...
            <?php foreach ($regions as $region): ?>
              <tr>
                ...
                <td><?= h($region->name) ?></td>
                <td><?= h($region->slug) ?></td>
                <td><?= $this->Number->format($region->status) ?></td>
                <td><?= h($region->created) ?></td>
                <td><?= h($region->modified) ?></td>
                <td class="actions" style="white-space:nowrap">
                  <?= $this->Html->link(__d('gentelella','Edit'), ['action' => 'edit', $region->id], ['class'=>'btn btn-primary btn-xs']) ?>
                  <?= $this->Form->postLink(__d('gentelella','Delete'), ['action' => 'delete', $region->id], ['confirm' => __d('gentelella','Confirm to delete this entry?'), 'class'=>'btn btn-danger btn-xs']) ?>
                </td>
              </tr>
            <?php endforeach; ?>
          </table>
