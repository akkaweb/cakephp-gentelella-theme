...
<!-- Main content -->
  <div class="row">
    <div class="col-xs-12">
      <div class="x_panel">
          <div class="x_title">
              <h2>Business <small><?= __d('gentelella','Index') ?></small></h2>
          ...
          <table class="table table-hover table-striped">
            ...
            <?php foreach ($businesses as $business): ?>
              <tr>
                ...
                <td><?= $this->Number->format($business->business_category_id) ?></td>
                <td><?= h($business->address) ?></td>
                <td><?= h($business->phone_number) ?></td>
                <td><?= h($business->website) ?></td>
                <td><?= h($business->email) ?></td>
                <td><?= $this->Number->format($business->publish) ?></td>
                <td class="actions" style="white-space:nowrap">
                  <?= $this->Html->link(__d('gentelella','Edit'), ['action' => 'edit', $business->id], ['class'=>'btn btn-primary btn-xs']) ?>
                  <?= $this->Form->postLink(__d('gentelella','Delete'), ['action' => 'delete', $business->id], ['confirm' => __d('gentelella','Confirm to delete this entry?'), 'class'=>'btn btn-danger btn-xs']) ?>
                </td>
              </tr>
            <?php endforeach; ?>
          </table>
