...
<!-- Main content -->
  <div class="row">
    <div class="col-xs-12">
      <div class="x_panel">
          <div class="x_title">
              <h2>Blog Category <small><?= __d('gentelella','Index') ?></small></h2>
          ...
          <table class="table table-hover table-striped">
            ...
            <?php foreach ($blogCategories as $blogCategory): ?>
              <tr>
                ...
                <td><?= $this->Number->format($blogCategory->parent_id) ?></td>
                <td><?= h($blogCategory->name) ?></td>
                <td><?= $this->Number->format($blogCategory->lft) ?></td>
                <td><?= $this->Number->format($blogCategory->rght) ?></td>
                <td><?= h($blogCategory->slug) ?></td>
                <td><?= h($blogCategory->created) ?></td>
                <td class="actions" style="white-space:nowrap">
                  <?= $this->Html->link(__d('gentelella','Edit'), ['action' => 'edit', $blogCategory->id], ['class'=>'btn btn-primary btn-xs']) ?>
                  <?= $this->Form->postLink(__d('gentelella','Delete'), ['action' => 'delete', $blogCategory->id], ['confirm' => __d('gentelella','Confirm to delete this entry?'), 'class'=>'btn btn-danger btn-xs']) ?>
                </td>
              </tr>
            <?php endforeach; ?>
          </table>
